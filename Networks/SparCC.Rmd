...
#phyloseq<-subset_samples(phyloseq, sample_names(phyloseq) != "S07")
#phyloseq<-subset_samples(phyloseq, sample_names(phyloseq) != "S09")

#Make year into factor
sample_data(phyloseq)$Year <- as.character(sample_data(phyloseq)$Year)
...
```{r}
system.time({
sparcc.samples <- sparcc(otu_table(phyloseq), iter=200,inner_iter=100)
str(sparcc.samples)
})
```
